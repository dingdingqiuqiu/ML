...
# (8) Train the sklearn decision tree with pruning
def train_sklearn_tree(df, max_depth=None, min_samples_split=2, min_samples_leaf=1):
...
max_depth = 3  # Example: limit the depth of the tree
min_samples_split = 4  # Example: minimum samples to split
min_samples_leaf = 2  # Example: minimum samples at a leaf node

library_clf = train_sklearn_tree(
    pd.DataFrame(data, columns=feature_names), 
    max_depth=max_depth, 
    min_samples_split=min_samples_split, 
    min_samples_leaf=min_samples_leaf
)  # Train library decision tree with pruning
